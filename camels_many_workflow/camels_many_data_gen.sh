...
#SBATCH --output=array_data_gen_%A-%a.log
...
sim_num=$1
sim_type=$(awk '{if(NR==4*n+1) print $0}' n=${sim_num} /orange/narayanan/d.zimmerman/camels_scripts/camels_config2.txt)
...
then
	sim_name="SIMBA"
	script="dataset_from_sim_gen_simba.py"
elif [ "$sim_type" = "1" ]
then
    	sim_name="IllustrisTNG"
	script="dataset_from_sim_gen_tng.py"
else
...
python ${script} $sim_id 67.11 $SLURM_ARRAY_TASK_ID
